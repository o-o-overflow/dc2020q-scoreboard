...
def last(psql, n):
    with psql.cursor() as cursor2:
        cursor2.execute('SELECT solves.date_created, team_name, challenge_id FROM solves,users WHERE solves.user_id = users.id order by solves.date_created desc limit %d;'%n)
...
def timeline(psql):
    ...
    for i in range(len(slots)):
        if i==current:
            print ("\x1b[32mNOW>\x1b[0m           %5s               %5s"%(slots[i][0], len(slots[i][1])))
        else:
...
def info_chall(psql, chall):
    ...
    with psql.cursor() as cursor2:
        cursor2.execute('SELECT solves.date_created, team_name FROM solves,users WHERE solves.user_id = users.id and challenge_id=%s;',(chall,))
...
def challs_table(psql):
    print ("")
    print ("\x1b[32m           Name       : Status  Solved  Wrong    Last  Open Since\x1b[0m")
    print ("\x1b[32m-----------------------------------------------------------------\x1b[0m")
    now = datetime.datetime.now(datetime.timezone.utc)
    with psql.cursor() as cursor:
        cursor.execute('SELECT id from challenges;')
        for name, in cursor.fetchall():
            ...
            with psql.cursor() as cursor2:
                cursor2.execute('SELECT date_created FROM challenges WHERE id=%s;', (name,))
                openfrom = "X"
                opentime = cursor2.fetchone()[0]
                diff = now-opentime
                openfrom = diff_string(diff)
            print ("\x1b[33m%20s\x1b[0m  :  OPEN    %3s    %5s %7s  %10s"%(name,tot,wrong,last,openfrom))
...
def teacheck(psql):
    with psql.cursor() as cursor:
        ...
        cursor.execute('SELECT date_created, user_id, challenge_id, flag FROM submissions WHERE challenge_id=\'throwback\' and user_id=177;')
...
def stats(psql):
    ...
    print ("\x1b[32m Open Challenges\x1b[0m:     %7s (%d%%)"%(open,open*100/(open+closed)))
